...
const makeDoc = (table: string, row: any, fallbackId?: string) => ({
  id: String(row?.id ?? fallbackId ?? ""),
  ref: doc(db, table, String(row?.id ?? fallbackId ?? "")),
  exists: () => Boolean(row),
  data: () => row || undefined,
});
...
export type SupabaseDB = typeof db;
...
export const collection = (_dbOrRef: unknown, ...segments: string[]): Ref => {
  ...
  return { kind: "collection", table, id: "", path: segments };
};

export const doc = (_dbOrRef: unknown, ...segments: string[]): Ref => {
  const id = String(segments[segments.length - 1]);
  const table = normalizeTable(segments.filter((_, index) => index % 2 === 0).join("_") || segments[0]);
...
export const getDoc = async (ref: Ref) => {
  ...
  return makeDoc(ref.table, data, ref.id);
};
...
export const getDocs = async (ref: Ref | QueryRef) => {
  ...
  return {
    ...
    size: docs.length,
    empty: docs.length === 0,
    forEach: (callback: (snapshot: any) => void) => docs.forEach(callback),
  };
};
...
export const runTransaction = async (_db: unknown, updateFn: (tx: Transaction) => Promise<void>) => {
  const txOps: (() => Promise<void>)[] = [];
  const tx: Transaction = {
    get: async (ref: Ref) => getDoc(ref),
    set: (ref: Ref, data: Record<string, any>, options?: { merge?: boolean }) => {
      txOps.push(() => setDoc(ref, data, options));
    },
    update: (ref: Ref, data: Record<string, any>) => {
      txOps.push(() => updateDoc(ref, data));
    },
    delete: (ref: Ref) => {
      txOps.push(() => deleteDoc(ref));
    },
  };
  await updateFn(tx);
...
export type Transaction = {
  get: (ref: Ref) => Promise<ReturnType<typeof makeDoc>>;
  set: (ref: Ref, data: Record<string, any>, options?: { merge?: boolean }) => void;
  update: (ref: Ref, data: Record<string, any>) => void;
  delete: (ref: Ref) => void;
};
